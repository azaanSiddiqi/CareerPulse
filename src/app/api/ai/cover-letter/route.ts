...
import { coverLetterSchema } from "@/lib/validation/ai";
...
export async function POST(request: Request) {
  ...
  if (!profile?.base_resume) {
    return apiError("invalid_input", "Add a base resume in Settings first.", 400);
  }
  ...
  let result;
  try {
    result = await provider.generateCoverLetter({
